...
class Env_simulation():
	...
	def place_adversaries(self,mu_sigmas):
		...
		while adversary_count < self.num_adversaries:
			# get mu/sigma from adversary network output
			#print("current adversary",adversary_count,"\n")
			adversary_mu_x = mu_sigmas[adversary_count*4]
			adversary_sigma_x = mu_sigmas[adversary_count*4 + 1]
			adversary_mu_y = mu_sigmas[adversary_count*4 + 2]
			adversary_sigma_y = mu_sigmas[adversary_count*4 + 3]
		
			adversary_x = np.random.normal(adversary_mu_x, adversary_sigma_x)
			adversary_y = np.random.normal(adversary_mu_y, adversary_sigma_y)
			...
			if self.animal_counts[row][col] > 0: #
	...
	def R_a(self):
		...
		nearby_adversary_animal_counts = convolve2d(animals_in_adversary_cells, kernel, \
			mode='same', boundary='fill')

		for defender in self.defender_coords:
			row = defender[0]
			col = defender[1]
			animals_defended = nearby_adversary_animal_counts[row][col]
