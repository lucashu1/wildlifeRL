...
adversary_output_tensor = defender_model.output
adversary_param_tensor = defender_model.trainable_weights
adverary_optimizer = tf.train.GradientDescentOptimizer(0.1)
adversary_gradients = optimizer.compute_gradients(defender_output_tensor, defender_param_tensor)
...
for i in range(1,5):
	...
	for t in range(1,10): #run the process through 1000 timesteps **
		...
		adversary_scaled_grad_var = [(tf.multiply(g,avg_reward),v) for g,v in gradients]
		
		adverary_update_weights = optimizer.apply_gradients(scaled_grad_var)
		
		sess.run(update_weights,feed_dict ={adversary_model.input:grid_vec})
	#chart.show_episode()
