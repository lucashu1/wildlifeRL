...
from keras.layers import Dense, Input, merge, Conv2D, Flatten, MaxPooling2D
...
defender_model.add(Conv2D(5, 3, 3, activation='relu', border_mode='same', \
	input_shape=(GRID_LEN, GRID_LEN, NUM_FEATURES)))
# defender_model.add(MaxPooling2D(pool_size=(2,2)))
defender_model.add(Flatten())
defender_model.add(Dense(256, init='uniform', activation='sigmoid'))
...
env =  Env_simulation(grid_len=GRID_LEN, num_defenders=NUM_DEFENDERS,\
		num_adversaries=NUM_ADVERSARIES, num_animals = NUM_ANIMALS, num_features=NUM_FEATURES)

chart = Reward_chart()

#create 5 episodes
for i in range(1,6):
	grid,animal_placements =  env.create_episode()
	...
	for t in range(1,101): #run the process through 1000 timesteps **
		...
		grid_vec = env.input_grid()
		defender_mu_sigma = defender_model.predict(grid_vec)[0]

		# print(defender_mu_sigma.shape)
